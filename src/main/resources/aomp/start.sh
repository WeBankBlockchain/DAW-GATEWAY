...
JAVA_OPTS+=" -XX:ErrorFile=/data/app/logs/wsdaw-gateway/java_error_%p.log -XX:+ExplicitGCInvokesConcurrent"
JAVA_OPTS+=" -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/data/app/logs/wsdaw-gateway"
JAVA_OPTS+=" -verbose:gc -Xloggc:/data/app/logs/wsdaw-gateway/jvm_gc.log"
JAVA_OPTS+=" -Djava.rmi.server.hostname=${HOST_IP}"

echo "nohup /data/wepkg/openjdk8u382-b05/bin/java $JAVA_OPTS -jar `pwd`/wsdaw-gateway_4395.jar >/dev/null 2>&1 &"
mkdir -p /data/app/logs/wsdaw-gateway
nohup /data/wepkg/openjdk8u382-b05/bin/java $JAVA_OPTS -jar `pwd`/wsdaw-gateway*.jar --spring.config.location=./config/application.yml &

sleep 8s
tail -n 150 /data/app/logs/wsdaw-gateway/wsdaw-gateway.log
